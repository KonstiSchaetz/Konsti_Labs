...
SELECT c.name,ROUND(AVG(length),2) AS running_time
 FROM film_category cf
	JOIN film f
    USING(film_id)
    JOIN category c
...
SELECT f.title, count(f.title), s.store_id,
CASE WHEN count(f.title) > 0 THEN 'yes' ELSE 'no' END AS availability
FROM inventory AS i
	JOIN film AS f
    USING(film_id)
    JOIN store AS s
    USING(store_id)
WHERE store_id = 1 AND title LIKE = '%Academy Dinosaurs%'
GROUP BY s.store_id;
...
CASE WHEN return_date IS NOT NULL THEN 'available' ELSE 'not available' END AS available
FROM film AS f
	INNER JOIN inventory AS i
	USING(film_id)
    RIGHT JOIN rental AS r
    USING (inventory_id)
GROUP BY title, available;
